 ...
 * <p>
 * ドメインレイヤーの Todo とインフラストラクチャ層の TodoEntity を調整し、
 * ビジネスロジックを実行する。履歴管理も併せて実施する。
 * </p>
 */
@Service
public class TodoService {
	 ...
	 * 新しい Todo を作成する。
	 * <p>
	 * ドメインの Todo.create() でビジネスロジックを検証し、
	 * リポジトリに永続化する。その後、履歴を記録する。
	 * </p>
	 *
	 ...
	 * 指定された Todo を論理削除する。
	 * <p>
	 * 公開ID で対象 Todo を検索し、削除フラグを立てる。
	 * 物理削除ではなく論理削除（ソフトデリート）を行う。
	 * </p>
	 *
	 * @param publicId 公開ID（UUID形式）
	 * @return 削除後の Todo
	...
	// ========================================================================
	/**
	 * 公開ID で指定された Todo を取得する。
	 * <p>
	 * 論理削除済みの Todo は存在しないものとして扱い、例外をスローする。
	 * </p>
	 *
	 * @param publicId 公開ID（UUID形式）
	 * @return 指定された Todo
	 ...
	 * <p>
	 * 期限日の昇順、その後作成日時の昇順でソートされた状態で返される。
	 * </p>
	 *
	 ...
	 * Todo の変更履歴を保存する。
	 * <p>
	 * 内部ID が設定されていない場合（JPA採番前）は保存をスキップする。
	 * </p>
	 *
